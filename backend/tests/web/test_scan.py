...
async def test_scan_endpoints():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        job["findings"] = [
            Finding(rule_id="R04", sheet="Sheet1", ref="A1", description="Test", severity="error", risk="display"),
            Finding(rule_id="R04", sheet="Sheet1", ref="A2", description="Test 2", severity="error", risk="value")
        ]
