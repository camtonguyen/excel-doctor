    ...
    for filename, data in meta.items():
        ...
        for key, expected_value in expected_inventory.items():
            if key == "requires_tier_a":
                assert actual_inventory.requires_tier_a == expected_value, (
                    f"Fixture {filename}: requires_tier_a mismatch"
                )
            else:
                actual_value = getattr(actual_inventory, key)
                assert actual_value == expected_value, (
                    f"Fixture {filename}: {key} mismatch. Expected {expected_value}, got {actual_value}"
                )
